...
class Piggy(PiggyParent):
    ...
    ****************
    ...
    def slither(self):
        ...
        for power in range(self.LEFT_DEFAULT, 75, -10):
            self.set_motor_power(self.MOTOR_LEFT, power)
            time.sleep(.5)
        # throttle up the left 
        for power in range(75, self.LEFT_DEFAULT + 1, 10):
            self.set_motor_power(self.MOTOR_LEFT, power)
            time.sleep(.1)

        # throttle down the right motor
        for power in range(self.RIGHT_DEFAULT, 75, -10):
            self.set_motor_power(self.MOTOR_RIGHT, power)
            time.sleep(.5)
        # throttle up the right 
        for power in range(75, self.RIGHT_DEFAULT + 1, 10):
            self.set_motor_power(self.MOTOR_RIGHT, power)
            time.sleep(.1)
