...
class Piggy(PiggyParent):
    ...
    ****************
    ...
    def slither(self):
        ...
        for power in range(self.LEFT_DEFAULT, 50, -5):
            self.set_motor_power(self.MOTOR_LEFT, power)
            time.sleep(.5)
        # throttle up the left 
        for power in range(50, self.LEFT_DEFAULT + 1, 5):
            self.set_motor_power(self.MOTOR_LEFT, power)
            time.sleep(.1)

        # throttle down the right motor
        for power in range(self.RIGHT_DEFAULT, 50, -5):
            self.set_motor_power(self.MOTOR_RIGHT, power)
            time.sleep(.5)
        # throttle up the right 
        for power in range(50, self.RIGHT_DEFAULT + 1, 5):
            self.set_motor_power(self.MOTOR_RIGHT, power)
            time.sleep(.1)
